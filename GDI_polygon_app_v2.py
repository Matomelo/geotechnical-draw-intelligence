...
def polygon_map_figure(results_df, map_image):
    """Colour the six polygon areas without adding duplicate A-F labels."""
    ...
    # Legend only; these traces do not add extra points to the map.
    for label, colour in [
        ("Good draw", "green"),
        ("Average / watch", "orange"),
        ("Poor draw", "red"),
        ("Waiting for data", "lightgrey"),
    ]:
        fig.add_trace(
            go.Scatter(
                x=[None],
    ...
    fig.update_layout(
        ...
        title="Lift II East Polygon Draw Status",
...
with tab_polygon:
    ...
    g1.metric("Good polygons", int(rating_counts.get("Green", 0)))
    g2.metric("Average / watch", int(rating_counts.get("Amber", 0)))
    g3.metric("Poor polygons", int(rating_counts.get("Red", 0)))
    g4.metric("Waiting for data", int(rating_counts.get("Grey", 0)))
