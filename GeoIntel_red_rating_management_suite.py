...
def red_rating_summary(df):
    ...
    return {
        "incidents": len(work),
        "hours": float(work["Duration Hours"].sum()),
        "open": open_count,
...
def red_rating_shift(start_text):
    """Classify a start time into Day, Afternoon or Night shift."""
    try:
        hour = datetime.strptime(str(start_text), "%H:%M").hour
    except Exception:
        return "Unknown"
    if 6 <= hour < 14:
        return "Day Shift"
...
def expand_red_rating_locations(df):
    """Return one row per affected location while preserving incident information."""
    if df.empty or "Locations" not in df.columns:
        return pd.DataFrame()
    work = df.copy()
    work["Duration Hours"] = pd.to_numeric(work.get("Duration Hours", 0), errors="coerce").fillna(0.0)
    work["Location"] = work["Locations"].fillna("").astype(str).str.split(",")
    work = work.explode("Location")
    work["Location"] = work["Location"].astype(str).str.strip()
    return work[work["Location"] != ""]


def red_rating_reliability_score(count, hours, open_count):
    """Return a 0-100 operational reliability score; higher is better."""
    penalty = min(60, count * 10) + min(30, hours * 2) + min(10, open_count * 5)
    return int(round(max(0, 100 - penalty)))


def red_rating_warning_text(location_summary, recent_days=10):
    if location_summary.empty:
        return []
    warnings = []
    for _, row in location_summary.iterrows():
        count = int(row.get("Red Ratings", 0))
        location = str(row.get("Location", ""))
        hours = float(row.get("Hours Lost", 0.0))
        if count >= 4:
            warnings.append(f"Recurring hotspot detected: {location} recorded {count} red ratings in the last {recent_days} days ({hours:.2f} h affected).")
        elif count >= 3:
            warnings.append(f"Watch recurring location: {location} recorded {count} red ratings in the last {recent_days} days.")
    return warnings
...
with tab_red_rating:
    st.header("Red Rating Tracker")
    st.caption("Capture seismic red-rating restrictions, calculate time lost automatically, and identify repeated problem areas.")

    capture_tab, excel_tab, dashboard_tab, history_tab = st.tabs(["➕ Capture", "📥 Excel Upload", "📊 Dashboard", "🗂️ History"])

    with capture_tab:
        with st.form("red_rating_capture_form", clear_on_submit=True):
            r1, r2, r3 = st.columns(3)
            incident_date = r1.date_input("Date", value=datetime.now().date(), key="rr_date")
            start_time = r2.time_input("Start time", value=datetime.now().replace(second=0, microsecond=0).time(), key="rr_start")
            end_time = r3.time_input("End time", value=datetime.now().replace(second=0, microsecond=0).time(), key="rr_end")

            locations = st.multiselect(
                "Affected locations",
                [f"PXC{i}" for i in range(1, 18)] + ["Lift II East", "Lift II Centre", "Lift II West", "Crusher 5", "Crusher 6", "Other"],
                key="rr_locations",
            )
            custom_location = st.text_input("Other location (optional)", key="rr_other_location")

            c1, c2, c3 = st.columns(3)
            category = c1.selectbox("Category", ["Seismic red rating", "Re-entry restriction", "Inspection delay", "System / communication delay", "Ground support", "Ventilation", "Other"], key="rr_category")
            activity = c2.selectbox("Activity affected", ["Production", "Development", "Support", "Inspection", "Travel / access", "Other"], key="rr_activity")
            status = c3.selectbox("Status", ["Open", "Under Review", "Closed"], key="rr_status")

            department = st.selectbox("Department / function", ["Geotechnical", "Mining", "Engineering", "Ventilation", "Survey", "Safety", "Other"], key="rr_department")
            trigger = st.text_input("Trigger / seismic event", key="rr_trigger")
            action_taken = st.text_area("Action taken / comments", height=100, key="rr_action")
            responsible = st.text_input("Responsible person", value=current_user if 'current_user' in globals() else "", key="rr_responsible")
            submit_red = st.form_submit_button("Save red-rating record", type="primary")

        minutes, hours = calculate_red_rating_duration(start_time, end_time)
        st.info(f"Calculated time lost: {minutes // 60} h {minutes % 60} min ({hours:.2f} h)")

        if submit_red:
            final_locations = list(locations)
            if custom_location.strip():
                final_locations.append(custom_location.strip())
            if not final_locations:
                st.error("Select or enter at least one affected location.")
            elif minutes <= 0:
                ...
                record = {
                    ...
                    "Duration Minutes": minutes,
                    "Duration Hours": round(hours, 3),
                    "Locations": ", ".join(final_locations),
                    "Category": category,
                    "Activity Affected": activity,
                    "Trigger": trigger,
                    "Action / Comments": action_taken,
                    "Responsible Person": responsible,
                    "Department": department,
                    "Status": status,
                    "Entered By": current_user if 'current_user' in globals() else "Unknown",
                    "Timestamp": datetime.now().isoformat(timespec="seconds"),
                }
                ...
                st.success("Red-rating record saved.")



    with excel_tab:
        st.subheader("Upload the approved Excel Red Rating Tracker")
        st.caption("Tailored for the 'Red Rating Delay Register' sheet and its Date, Crosscut / Location, Rating, start time, end time and Hours Lost columns.")
        red_excel = st.file_uploader(
            "Upload Red_Rating_Tracker_July_Corrected_II_UXC.xlsx",
            type=["xlsx", "xls"],
            key="red_rating_excel_upload",
        )
        if red_excel is not None:
            ...
            if imported_red.empty:
                ...
                preview_cols = ["Date", "Start Time", "End Time", "Duration Hours", "Locations", "Category", "Status"]
                ...
                p1, p2 = st.columns(2)
                p1.metric("Records ready", len(imported_red))
                p2.metric("Total hours", f"{total_import_hours:.2f} h")
                for warning in import_warnings:
                    st.warning(warning)
                if st.button("Import Excel records into GeoIntel", type="primary", use_container_width=True, key="import_red_excel_button"):
    ...
    red_df = load_csv(RED_RATING_FILE)
    ...
    with dashboard_tab:
        if red_df.empty:
            ...
            work["Duration Hours"] = pd.to_numeric(work.get("Duration Hours", 0), errors="coerce").fillna(0.0)
            work["Start Time"] = work.get("Start Time", "").astype(str)
            work["Shift"] = work["Start Time"].map(red_rating_shift)

            st.markdown("### Red Rating KPI Dashboard")
            set1, set2 = st.columns(2)
            production_rate = set1.number_input("Production rate (tonnes/hour)", min_value=0.0, value=500.0, step=10.0, key="rr_prod_rate")
            value_per_tonne = set2.number_input("Estimated value per tonne (R)", min_value=0.0, value=0.0, step=10.0, key="rr_value_tonne")

            today = pd.Timestamp.today().normalize()
            month_start = today.replace(day=1)
            year_start = today.replace(month=1, day=1)
            today_hours = work.loc[work["Date"].dt.normalize() == today, "Duration Hours"].sum()
            month_hours = work.loc[work["Date"] >= month_start, "Duration Hours"].sum()
            year_hours = work.loc[work["Date"] >= year_start, "Duration Hours"].sum()
            total_hours = work["Duration Hours"].sum()
            total_tonnes = total_hours * production_rate
            total_cost = total_tonnes * value_per_tonne
            open_mask = work.get("Status", pd.Series(index=work.index, dtype=str)).astype(str).isin(["Open", "Under Review"])

            k1, k2, k3, k4, k5 = st.columns(5)
            k1.metric("Red ratings", len(work))
            k2.metric("Today", f"{today_hours:.2f} h")
            k3.metric("This month", f"{month_hours:.2f} h")
            k4.metric("Estimated tonnes lost", f"{total_tonnes:,.0f} t")
            k5.metric("Revenue at risk", f"R {total_cost:,.0f}")
            st.caption(f"Year-to-date time lost: {year_hours:.2f} h • Open / under review: {int(open_mask.sum())}")

            expanded = expand_red_rating_locations(work)
            if not expanded.empty:
                hotspot = expanded.groupby("Location", as_index=False).agg(
                    **{"Red Ratings": ("Record ID", "nunique"), "Hours Lost": ("Duration Hours", "sum")}
                )
                hotspot["Estimated Tonnes Lost"] = hotspot["Hours Lost"] * production_rate
                hotspot["Open Items"] = expanded.assign(_open=expanded.get("Status", "").astype(str).isin(["Open", "Under Review"])).groupby("Location")["_open"].sum().reindex(hotspot["Location"]).values
                hotspot["Reliability Score"] = hotspot.apply(lambda r: red_rating_reliability_score(r["Red Ratings"], r["Hours Lost"], r["Open Items"]), axis=1)
                hotspot = hotspot.sort_values(["Red Ratings", "Hours Lost"], ascending=False)

                st.markdown("### Live Hotspot Ranking")
                top = hotspot.head(10).copy()
                top["Rank"] = range(1, len(top) + 1)
                st.dataframe(top[["Rank", "Location", "Red Ratings", "Hours Lost", "Estimated Tonnes Lost", "Reliability Score"]], width="stretch", hide_index=True)

                hleft, hright = st.columns(2)
                with hleft:
                    fig_hot = px.bar(top.sort_values("Hours Lost"), x="Hours Lost", y="Location", orientation="h", title="Top hotspot locations by affected hours", text="Red Ratings")
                    fig_hot.update_layout(height=420, margin=dict(l=10, r=10, t=50, b=10))
                    st.plotly_chart(fig_hot, width="stretch")
                with hright:
                    fig_heat = px.treemap(top, path=["Location"], values="Hours Lost", color="Red Ratings", title="Red Rating Heat Map")
                    fig_heat.update_layout(height=420, margin=dict(l=10, r=10, t=50, b=10))
                    st.plotly_chart(fig_heat, width="stretch")

                recent_cutoff = today - pd.Timedelta(days=10)
                recent_expanded = expanded[expanded["Date"] >= recent_cutoff] if "Date" in expanded.columns else pd.DataFrame()
                if not recent_expanded.empty:
                    recent_summary = recent_expanded.groupby("Location", as_index=False).agg(**{"Red Ratings": ("Record ID", "nunique"), "Hours Lost": ("Duration Hours", "sum")}).sort_values("Red Ratings", ascending=False)
                    warnings = red_rating_warning_text(recent_summary, 10)
                    if warnings:
                        st.markdown("### Predictive / Recurrence Warnings")
                        for warning in warnings:
                            st.warning(warning)

            a1, a2 = st.columns(2)
            with a1:
                cause_col = "Trigger" if "Trigger" in work.columns and work["Trigger"].fillna("").astype(str).str.strip().ne("").any() else "Category"
                cause = work.copy()
                cause[cause_col] = cause[cause_col].fillna("Not specified").astype(str).replace("", "Not specified")
                cause_summary = cause.groupby(cause_col, as_index=False)["Duration Hours"].sum().sort_values("Duration Hours", ascending=False)
                fig_cause = px.pie(cause_summary, names=cause_col, values="Duration Hours", title="Cause Analysis — share of hours lost", hole=0.42)
                fig_cause.update_layout(height=390, margin=dict(l=10, r=10, t=50, b=10))
                st.plotly_chart(fig_cause, width="stretch")
            with a2:
                shift_summary = work.groupby("Shift", as_index=False).agg(Incidents=("Record ID", "nunique"), **{"Hours Lost": ("Duration Hours", "sum")})
                fig_shift = px.bar(shift_summary, x="Shift", y="Hours Lost", text="Incidents", title="Shift Analysis")
                fig_shift.update_layout(height=390, margin=dict(l=10, r=10, t=50, b=10))
                st.plotly_chart(fig_shift, width="stretch")

            b1, b2 = st.columns(2)
            with b1:
                if "Department" in work.columns and work["Department"].fillna("").astype(str).str.strip().ne("").any():
                    dept = work.copy()
                    dept["Department"] = dept["Department"].fillna("Not specified").astype(str).replace("", "Not specified")
                    dept_summary = dept.groupby("Department", as_index=False).agg(Incidents=("Record ID", "nunique"), **{"Hours Lost": ("Duration Hours", "sum")}).sort_values("Hours Lost", ascending=False)
                    fig_dept = px.bar(dept_summary, x="Department", y="Hours Lost", text="Incidents", title="Department / Function Analysis")
                    fig_dept.update_layout(height=370, margin=dict(l=10, r=10, t=50, b=10))
                    st.plotly_chart(fig_dept, width="stretch")
                else:
                    st.info("Department analysis will populate as new records are captured with a department/function.")
            with b2:
                open_records = work[open_mask].copy()
                if not open_records.empty:
                    open_records["Elapsed Hours"] = open_records.apply(lambda r: max(float(r.get("Duration Hours", 0)), (pd.Timestamp.now() - r["Date"]).total_seconds() / 3600 if not pd.isna(r["Date"]) else 0), axis=1)
                    longest = open_records.sort_values("Elapsed Hours", ascending=False).iloc[0]
                    st.markdown("### Longest Open Red Rating")
                    st.metric(str(longest.get("Locations", "Unknown location")), f"{longest['Elapsed Hours']:.1f} h")
                    st.write(f"Status: **{longest.get('Status', 'Open')}**")
                    st.write(f"Started: **{longest.get('Date', '')} {longest.get('Start Time', '')}**")
                else:
                    st.success("No open red ratings.")

            daily = work.dropna(subset=["Date"]).groupby("Date", as_index=False)["Duration Hours"].sum()
            if not daily.empty:
                fig_daily = px.line(daily, x="Date", y="Duration Hours", markers=True, title="Red-rating time-loss trend")
                fig_daily.update_layout(height=360, margin=dict(l=10, r=10, t=50, b=10))
    ...
    with history_tab:
        if red_df.empty:
            ...
            date_filter = h1.date_input("From date", value=pd.to_datetime(red_df["Date"], errors="coerce").min().date() if pd.to_datetime(red_df["Date"], errors="coerce").notna().any() else datetime.now().date(), key="rr_from")
            status_filter = h2.multiselect("Status", sorted(red_df["Status"].dropna().astype(str).unique()), key="rr_status_filter") if "Status" in red_df.columns else []
            search_location = h3.text_input("Location contains", key="rr_location_search")
            filtered = red_df.copy()
            dates = pd.to_datetime(filtered["Date"], errors="coerce")
            filtered = filtered[dates.dt.date >= date_filter]
            if status_filter:
                filtered = filtered[filtered["Status"].astype(str).isin(status_filter)]
            if search_location.strip():
                filtered = filtered[filtered["Locations"].astype(str).str.contains(search_location.strip(), case=False, na=False)]
            st.dataframe(filtered.sort_values(["Date", "Start Time"], ascending=False), width="stretch", hide_index=True)
