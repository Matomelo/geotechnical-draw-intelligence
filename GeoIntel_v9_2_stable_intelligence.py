...
def geointel_readiness_checks(config):
    """Return deployment-readiness checks without blocking the dashboard."""
    checks = []
    checks.append((DEFAULT_MAP_FILE.exists(), "Lift II East map file found" if DEFAULT_MAP_FILE.exists() else "Map file is missing"))
    enabled = [n for n, v in config.items() if v.get("enabled") and to_float(v.get("weight", 0)) > 0]
    checks.append((bool(enabled), f"Risk engine active: {', '.join(enabled)}" if enabled else "No risk components are enabled"))
    checks.append(("Convergence" not in enabled or CONVERGENCE_FILE.exists(), "Convergence input available or intentionally excluded" if "Convergence" not in enabled or CONVERGENCE_FILE.exists() else "Convergence is enabled but no saved convergence file exists yet"))
    checks.append((True, "User roles and approval permissions loaded"))
    return checks


def lift_traffic_light(results_df):
    if results_df.empty:
        return "WAITING", "#64748b", "No polygon results available"
    risks = pd.to_numeric(results_df.get("Combined Risk Index", 0), errors="coerce").fillna(0)
    ratings = results_df.get("Map Rating", pd.Series(dtype=str)).astype(str)
    if (risks >= 75).any() or (ratings == "Red").any():
        return "CRITICAL", "#dc2626", "At least one polygon requires immediate attention"
    if (risks >= 50).any() or (ratings == "Amber").any():
        return "WATCH", "#d97706", "One or more polygons require closer monitoring"
    if (ratings == "Grey").all():
        return "WAITING", "#64748b", "Core data is still incomplete"
    return "GOOD", "#15803d", "No high-priority polygon conditions detected"


def assessment_confidence(row, mapping_df, config):
    checks = {
        "Seismic events/energy": to_float(row.get("Energy J", 0)) > 0 or to_int(row.get("Events", 0)) > 0,
        "Potency": to_float(row.get("Potency m3", 0)) > 0,
        "Production tonnes": to_float(row.get("Polygon Tonnes", 0)) > 0,
        "Drawpoint mapping": not mapping_df.empty,
    }
    conv_enabled = bool(config.get("Convergence", {}).get("enabled", False))
    checks["Convergence"] = (not conv_enabled) or str(row.get("Convergence Risk", "No convergence data")) != "No convergence data"
...
def polygon_intelligence_text(row, trend_label, confidence):
    ...
    if status in ["Brittle but Efficient", "High-Risk Draw"]:
        drivers.append("elevated E/P")
    if status in ["Ductile but Costly", "High-Risk Draw"]:
        drivers.append("elevated E/T")
    ach = row.get("Achievement %", None)
    if ach is not None and not pd.isna(ach) and to_float(ach) < 80:
        drivers.append("production below target")
    if str(row.get("Convergence Risk", "")) in ["High convergence", "Combined geotechnical concern"]:
        drivers.append("elevated convergence response")
    driver_text = ", ".join(drivers) if drivers else "no dominant adverse driver identified"
    return (
        f"Current status is {status} with {risk.lower()} combined risk. "
        f"The main interpretation is {driver_text}. The health trend is {trend_label.lower()} "
        f"and assessment confidence is {confidence}%. {row.get('Decision Recommendation', '')}"
    )


def seven_day_timeline(history_df, polygon, current_row):
    rows = []
    if not history_df.empty and "Polygon" in history_df.columns and "Report Date" in history_df.columns:
        hist = history_df[history_df["Polygon"].astype(str).str.upper() == str(polygon).upper()].copy()
        hist["_date"] = pd.to_datetime(hist["Report Date"], errors="coerce")
        hist = hist.dropna(subset=["_date"]).sort_values("_date").tail(6)
        for _, r in hist.iterrows():
            rows.append({"Date": r["_date"].strftime("%d %b"), "Status": r.get("Status", "Waiting"), "Health": to_int(r.get("Health Score", 0)), "Risk": r.get("Combined Risk", "—")})
    rows.append({"Date": "Current", "Status": current_row.get("Status", "Waiting"), "Health": to_int(current_row.get("Health Score", 0)), "Risk": current_row.get("Combined Risk", "—")})
    return pd.DataFrame(rows).tail(7)


def daily_intelligence_feed(results_df, history_df):
    feed = []
    if results_df.empty:
        return ["No current polygon results are available."]
    highest = results_df.sort_values("Combined Risk Index", ascending=False).iloc[0]
    feed.append(f"Polygon {highest['Polygon']} has the highest current combined risk at {to_int(highest['Combined Risk Index'])}/100 ({highest['Combined Risk']}).")
    red = results_df[results_df["Map Rating"] == "Red"]
    if not red.empty:
        feed.append("Poor draw rating active in polygon(s): " + ", ".join(red["Polygon"].astype(str)) + ".")
    amber = results_df[results_df["Map Rating"] == "Amber"]
    if not amber.empty:
        feed.append("Watch rating active in polygon(s): " + ", ".join(amber["Polygon"].astype(str)) + ".")
    if not history_df.empty and "Health Score" in history_df.columns and "Polygon" in history_df.columns:
        for _, row in results_df.iterrows():
            label, change, _ = polygon_trend(history_df, row["Polygon"], row["Health Score"])
            if abs(change) >= 5:
                feed.append(f"Polygon {row['Polygon']} is {label.lower()} by {abs(change):.0f} health points versus its latest saved record.")
    if len(feed) == 1:
        feed.append("No major rating transitions are identified from the available history.")
    return feed[:6]
...
with st.sidebar.expander("System readiness", expanded=False):
    for passed, message in geointel_readiness_checks(risk_engine_config):
        st.success(message) if passed else st.warning(message)
...
.intel-card{background:#fff;border:1px solid var(--border);border-radius:10px;padding:.8rem .9rem;margin:.35rem 0;box-shadow:0 1px 3px rgba(15,23,42,.05);}
.traffic-card{color:white;border-radius:10px;padding:.75rem 1rem;margin:.25rem 0 .7rem 0;}
.traffic-title{font-size:.75rem;font-weight:800;opacity:.9}.traffic-value{font-size:1.65rem;font-weight:900;line-height:1.15}.small-muted{color:#64748b;font-size:.82rem;}
...
<div class="gdi-header">
  <div><div class="gdi-header-title">GeoIntel — GEOTECHNICAL DECISION INTELLIGENCE</div><div class="gdi-header-sub">Lift II East Draw, Seismic & Geotechnical Decision Support</div></div>
...
with tab_polygon:
    ...
    overall_status, overall_colour, overall_reason = lift_traffic_light(dashboard_results)
    overall_risk = int(pd.to_numeric(dashboard_results["Combined Risk Index"], errors="coerce").fillna(0).mean()) if not dashboard_results.empty else 0
    st.markdown(
        f'<div class="traffic-card" style="background:{overall_colour}"><div class="traffic-title">OVERALL LIFT II EAST STATUS</div><div class="traffic-value">{overall_status}</div><div>Average risk {overall_risk}/100 · {overall_reason}</div></div>',
        unsafe_allow_html=True,
    )
    with st.expander("Today’s intelligence feed", expanded=True):
        for item in daily_intelligence_feed(dashboard_results, polygon_history):
            st.write(f"• {item}")
    ...
    with right:
        ...
        confidence_score, confidence_checks = assessment_confidence(selected, mapping_df, risk_engine_config)
        intelligence_text = polygon_intelligence_text(selected, trend_label, confidence_score)
        st.markdown('<div class="section-bar">GEOINTEL ANALYSIS</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="intel-card"><b>Polygon intelligence</b><br>{intelligence_text}</div>', unsafe_allow_html=True)
        st.metric("Assessment confidence", f"{confidence_score}%")
        with st.expander("Confidence basis", expanded=False):
            for item, passed in confidence_checks.items():
                st.write(("✓ " if passed else "✗ ") + item)
        with st.expander("7-day geotechnical timeline", expanded=False):
            st.dataframe(seven_day_timeline(polygon_history, selected_poly, selected), width="stretch", hide_index=True)
