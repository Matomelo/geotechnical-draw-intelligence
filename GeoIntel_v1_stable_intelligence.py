...
def selected_polygon_report(row, trend_label, trend_change, explanation, alerts):
    ...
    return build_word_report(f"GeoIntel Polygon {poly} Decision Support Report", summary, report_data)
...
def build_dashboard_report(report_date, results_df, quality_issues, executive_text):
    ...
    doc = Document()
    doc.add_heading("GeoIntel Lift II East Management Report", 0)
...
def build_printable_html_report(report_date, results_df, executive_text, quality_issues):
    ...
    html = (
        '<!doctype html><html><head><meta charset="utf-8"><title>GeoIntel Report</title>'
        ...
        f'</head><body><h1>GeoIntel — Lift II East Management Report</h1><p><b>Reporting date:</b> {report_date}</p>'
...
def polygon_confidence_score(row, mapping_df, risk_config):
    """Return a transparent 0-100 confidence score and reasons."""
    checks = []
    score = 0
    energy = to_float(row.get("Energy J", 0))
    potency = to_float(row.get("Potency m3", 0))
    tonnes = to_float(row.get("Polygon Tonnes", 0))
    events = to_int(row.get("Events", 0))

    if energy > 0 or events > 0:
        score += 20; checks.append((True, "Seismic data available"))
    else:
        checks.append((False, "Seismic data missing"))
    if potency > 0:
        score += 20; checks.append((True, "Potency available"))
    else:
        checks.append((False, "Potency missing"))
    if tonnes > 0:
        score += 20; checks.append((True, "Production tonnes available"))
    else:
        checks.append((False, "Production tonnes missing"))
    mapping_ok = not mapping_df.empty and "Polygon" in mapping_df.columns and not mapping_df["Polygon"].astype(str).str.strip().eq("").any()
    if mapping_ok:
        score += 20; checks.append((True, "Drawpoint mapping complete"))
    else:
        checks.append((False, "Drawpoint mapping incomplete"))

    convergence_enabled = bool(risk_config.get("Convergence", {}).get("enabled", False)) and to_float(risk_config.get("Convergence", {}).get("weight", 0)) > 0
    if not convergence_enabled:
        score += 20; checks.append((True, "Convergence intentionally excluded"))
    else:
        closure = to_float(row.get("Closure mm", 0))
        rate = to_float(row.get("Rate mm/day", 0))
        if closure > 0 or rate > 0:
            score += 20; checks.append((True, "Convergence data available"))
        else:
            checks.append((False, "Convergence enabled but data missing"))
    return int(min(100, score)), checks


def lift2east_traffic_light(results_df):
    """Return overall status, colour, and a concise reason."""
    if results_df.empty:
        return "NO DATA", "#64748b", "No polygon results available"
    risks = pd.to_numeric(results_df.get("Combined Risk Index", 0), errors="coerce").fillna(0)
    red = int((results_df.get("Map Rating", "") == "Red").sum())
    amber = int((results_df.get("Map Rating", "") == "Amber").sum())
    critical = int((results_df.get("Combined Risk", "") == "Critical").sum())
    high = int((results_df.get("Combined Risk", "") == "High").sum())
    if critical > 0 or red >= 2 or risks.max() >= 75:
        return "CRITICAL", "#b91c1c", f"{critical} critical, {red} poor polygon(s)"
    if high > 0 or red > 0 or amber >= 2 or risks.max() >= 50:
        return "WATCH", "#d97706", f"{high} high-risk, {amber} watch polygon(s)"
    if (results_df.get("Map Rating", "") == "Grey").all():
        return "WAITING FOR DATA", "#64748b", "Core data have not yet been entered"
    return "GOOD", "#15803d", "No high-priority polygon exceptions"


def polygon_timeline(history_df, polygon, current_row=None, days=7):
    """Return recent dated status rows for one polygon, optionally including current input."""
    rows = []
    if not history_df.empty and {"Polygon", "Report Date"}.issubset(history_df.columns):
        hist = history_df[history_df["Polygon"].astype(str).str.upper() == str(polygon).upper()].copy()
        if not hist.empty:
            hist["_date"] = pd.to_datetime(hist["Report Date"], errors="coerce")
            hist = hist.dropna(subset=["_date"]).sort_values("_date").tail(days)
            for _, r in hist.iterrows():
                rows.append({"Date": r["_date"].date().isoformat(), "Status": str(r.get("Status", "No data")), "Health": to_int(r.get("Health Score", 0)), "Risk": to_int(r.get("Combined Risk Index", 0))})
    if current_row is not None:
        current_date = str(current_row.get("Report Date", "Current"))
        rows.append({"Date": current_date, "Status": str(current_row.get("Status", "No data")), "Health": to_int(current_row.get("Health Score", 0)), "Risk": to_int(current_row.get("Combined Risk Index", 0))})
    return pd.DataFrame(rows).drop_duplicates(subset=["Date"], keep="last").tail(days) if rows else pd.DataFrame(columns=["Date", "Status", "Health", "Risk"])


def polygon_intelligence_text(row, trend_label, confidence):
    """Interpret the most likely current driver and action."""
    status = str(row.get("Status", "Waiting for data"))
    risk = str(row.get("Combined Risk", "Low"))
    ep = to_float(row.get("E/P", 0))
    etv = row.get("E/T", None)
    et = None if etv in [None, ""] or pd.isna(etv) else to_float(etv)
    achievement = row.get("Achievement %", None)
    conv = str(row.get("Convergence Risk", "No convergence data"))
    drivers = []
    if status in ["Brittle but Efficient", "High-Risk Draw"]:
        drivers.append(f"elevated E/P ({ep:,.2f})")
    if status in ["Ductile but Costly", "High-Risk Draw"] and et is not None:
        drivers.append(f"elevated E/T ({et:,.2f})")
    if achievement is not None and not pd.isna(achievement) and to_float(achievement) < 80:
        drivers.append(f"low production achievement ({to_float(achievement):.1f}%)")
    if conv in ["Convergence watch", "High convergence", "Combined geotechnical concern"]:
        drivers.append(conv.lower())
    if not drivers:
        drivers.append("no dominant adverse driver identified")
    driver_text = ", ".join(drivers)
    action = str(row.get("Decision Recommendation", "Maintain routine monitoring."))
    return f"Current assessment: {risk} risk. Most likely driver: {driver_text}. Trend: {trend_label.lower()}. Confidence: {confidence}%. Recommended action: {action}"


def daily_intelligence_feed(results_df, history_df, report_date):
    """Generate short daily highlights by comparing with the latest earlier saved date."""
    highlights = []
    if results_df.empty:
        return ["No current polygon results available."]
    current = results_df.copy()
    current["Polygon"] = current["Polygon"].astype(str)
    previous = pd.DataFrame()
    if not history_df.empty and {"Report Date", "Polygon"}.issubset(history_df.columns):
        hist = history_df.copy()
        hist["_date"] = pd.to_datetime(hist["Report Date"], errors="coerce")
        target = pd.to_datetime(str(report_date), errors="coerce")
        prior_dates = hist.loc[hist["_date"] < target, "_date"].dropna()
        if not prior_dates.empty:
            prev_date = prior_dates.max()
            previous = hist[hist["_date"] == prev_date].copy()
    if not previous.empty:
        merged = current.merge(previous, on="Polygon", suffixes=("", "_prev"), how="left")
        for _, r in merged.iterrows():
            h_now = to_float(r.get("Health Score", 0)); h_prev = to_float(r.get("Health Score_prev", h_now))
            delta = h_now - h_prev
            if abs(delta) >= 10:
                highlights.append(f"Polygon {r['Polygon']} health {'improved' if delta > 0 else 'deteriorated'} by {abs(delta):.0f} points.")
            rating_now = str(r.get("Map Rating", "")); rating_prev = str(r.get("Map Rating_prev", rating_now))
            if rating_now != rating_prev:
                highlights.append(f"Polygon {r['Polygon']} changed from {rating_prev or 'No data'} to {rating_now or 'No data'}.")
    risk_rank = current.sort_values("Combined Risk Index", ascending=False)
    top = risk_rank.iloc[0]
    highlights.append(f"Highest current risk is Polygon {top['Polygon']} at {to_int(top.get('Combined Risk Index', 0))}/100 ({top.get('Combined Risk', 'Low')}).")
    red_count = int((current.get("Map Rating", "") == "Red").sum())
    if red_count == 0:
        highlights.append("No polygons are currently rated poor on the E/P–E/T matrix.")
    else:
        highlights.append(f"{red_count} polygon(s) are currently rated poor and require attention.")
    return highlights[:6]

def geointel_readiness_checks(risk_config):
    """Return lightweight startup checks for the stable release."""
    checks = []
    checks.append((DEFAULT_MAP_FILE.exists(), "Lift II East map file available"))
    enabled = [name for name, cfg in risk_config.items() if cfg.get("enabled") and to_float(cfg.get("weight", 0)) > 0]
    checks.append((bool(enabled), "At least one risk component enabled"))
    checks.append((DEFAULT_EP_THRESHOLD > 0 and DEFAULT_ET_THRESHOLD > 0, "Default E/P and E/T thresholds valid"))
    checks.append((len(set(enabled)) == len(enabled), "Risk components are unique"))
    return checks
...
with st.sidebar.expander("System readiness", expanded=False):
    for passed, message in geointel_readiness_checks(risk_engine_config):
        st.caption(("✓ " if passed else "✗ ") + message)
    st.caption("Convergence may remain OFF while trials are incomplete; disabled components are excluded and enabled weights are normalised.")
...
with tab_polygon:
    ...
    with st.expander("Calculation and data-flow proof", expanded=False):
        required_cols = ["Polygon", "Energy J", "Potency m3", "Polygon Tonnes", "E/P", "E/T", "Status", "Map Rating", "Health Score", "Combined Risk Index"]
        missing_cols = [c for c in required_cols if c not in results.columns]
        if missing_cols:
            st.error("Missing calculated fields: " + ", ".join(missing_cols))
        else:
            st.success("Production → drawbell → polygon totals and seismic → E/P/E/T → matrix → map colour → risk index are connected.")
        st.caption("Convergence is used only when switched ON in the Configurable Risk Engine.")
    ...
    # V9.1 Intelligence layer: traffic light, confidence, timeline, intelligence and daily feed.
    overall_label, overall_colour, overall_reason = lift2east_traffic_light(dashboard_results)
    st.markdown(
        f'<div style="background:{overall_colour};color:white;border-radius:10px;padding:.8rem 1rem;margin:.25rem 0 .7rem 0;display:flex;justify-content:space-between;align-items:center;">'
        f'<div><div style="font-size:.8rem;font-weight:700;opacity:.9;">OVERALL LIFT II EAST STATUS</div><div style="font-size:1.55rem;font-weight:900;">{overall_label}</div></div>'
        f'<div style="font-weight:700;text-align:right;">{overall_reason}</div></div>',
        unsafe_allow_html=True,
    )

    selected_for_intel = dashboard_results[dashboard_results["Polygon"] == st.session_state.selected_polygon].iloc[0]
    confidence_score, confidence_checks = polygon_confidence_score(selected_for_intel, mapping_df, risk_engine_config)
    intel_trend_label, intel_trend_change, intel_trend_arrow = polygon_trend(
        polygon_history, st.session_state.selected_polygon, selected_for_intel.get("Health Score", 0)
    )
    intel_text = polygon_intelligence_text(selected_for_intel, intel_trend_label, confidence_score)

    intel1, intel2 = st.columns([1.15, 1])
    with intel1:
        st.markdown('<div class="section-bar">POLYGON INTELLIGENCE</div>', unsafe_allow_html=True)
        st.info(intel_text)
        st.caption(f"Selected polygon: {st.session_state.selected_polygon} | Trend {intel_trend_arrow} {intel_trend_label} ({intel_trend_change:+.0f})")
    with intel2:
        st.markdown('<div class="section-bar">ASSESSMENT CONFIDENCE</div>', unsafe_allow_html=True)
        st.metric("Confidence", f"{confidence_score}%")
        st.progress(confidence_score)
        for passed, reason in confidence_checks:
            st.caption(("✓ " if passed else "✗ ") + reason)

    timeline_df = polygon_timeline(polygon_history, st.session_state.selected_polygon, selected_for_intel, days=7)
    st.markdown('<div class="section-bar">7-DAY GEOTECHNICAL TIMELINE</div>', unsafe_allow_html=True)
    if timeline_df.empty:
        st.caption("Save dated polygon results to build the timeline.")
    else:
        icon_map = {"Efficient Draw":"🟢", "Brittle but Efficient":"🟠", "Ductile but Costly":"🟠", "High-Risk Draw":"🔴", "Waiting for tonnes":"⚪", "Waiting for seismic data":"⚪"}
        cols = st.columns(len(timeline_df))
        for idx, (_, item) in enumerate(timeline_df.iterrows()):
            cols[idx].markdown(f"**{item['Date']}**  \n{icon_map.get(item['Status'],'⚪')} {item['Status']}  \nHealth {to_int(item['Health'])} | Risk {to_int(item['Risk'])}")

    st.markdown('<div class="section-bar">DAILY INTELLIGENCE FEED</div>', unsafe_allow_html=True)
    for highlight in daily_intelligence_feed(dashboard_results, polygon_history, replay_date or polygon_date):
    ...
    with right:
        ...
        st.download_button(
            ...
            data=report_file,
            file_name=f"GeoIntel_Polygon_{selected_poly}_{polygon_date}.docx",
    ...
    st.download_button(
        ...
        file_name=f"GeoIntel_Lift_II_East_{replay_date or polygon_date}.docx",
        mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
    ...
    st.download_button(
        ...
        file_name=f"GeoIntel_Lift_II_East_{replay_date or polygon_date}.html",
            ...
            if "Lift_II_E" in poly_data:
                ...
                if liie["Potency m3"] > 0:
                    ...
                    st.download_button(
                        ...
                        data=word_file,
                        file_name="GeoIntel_Weekly_Report.docx",
                        mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                    )
            else:
