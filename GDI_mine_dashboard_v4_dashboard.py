...
DEFAULT_MAP_FILE = Path("lift2east_polygon_map_clean.png")
...
def polygon_map_figure(results_df, map_image, selected_polygon="A"):
    """Clean status map with coloured polygon fills and small labels."""
    ...
    for poly, (x0r, y0r, x1r, y1r) in boxes.items():
        ...
        fig.add_shape(
            ...
            fillcolor=colour, opacity=0.22 if not selected else 0.34,
            line=dict(color=colour, width=6 if selected else 3), layer="above"
        )
        ...
        fig.add_trace(go.Scatter(
            ...
            text=[poly], textposition="middle center",
            textfont=dict(size=20, color="#111827"),
            marker=dict(size=88, opacity=0), hovertext=[hover], hoverinfo="text",
            showlegend=False
        ))
    ...
    fig.update_layout(
        height=760, margin=dict(l=0, r=0, t=0, b=0), title=None,
...
with tab_polygon:
    ...
    st.markdown(f"### Drawbells and drawpoints — Polygon {st.session_state.selected_polygon}")
