...
RED_RATING_TIMELINE_FILE = Path("geointel_red_rating_timeline.csv")
RED_RATING_ATTACHMENTS_DIR = Path("geointel_red_rating_attachments")
RED_RATING_EFFECTIVENESS_FILE = Path("geointel_red_rating_effectiveness.csv")
RED_RATING_LESSONS_FILE = Path("geointel_red_rating_lessons.csv")
RED_RATING_APPROVAL_FILE = Path("geointel_red_rating_approvals.csv")
...
def prepare_red_rating_data(df):
    ...
    for col, default in {
        ...
        "Investigation Stage":"Logged", "Trigger":"", "Action / Comments":"", "Outcome":"", "Responsible Person":""
    }.items():
...
def red_rating_severity(row):
    """Transparent 0-100 impact score based only on recorded incident fields."""
    hours = max(0.0, float(pd.to_numeric(pd.Series([row.get("Duration Hours", 0)]), errors="coerce").fillna(0).iloc[0]))
    locations = [x.strip() for x in str(row.get("Locations", "")).split(",") if x.strip()]
    category = str(row.get("Category", "Unspecified"))
    status = str(row.get("Status", "Closed"))
    duration_score = min(45, hours / 8.0 * 45)
    location_score = min(25, len(locations) * 5)
    category_score = {"Seismic": 20, "Ground Support": 18, "Ventilation": 15, "Electrical": 15, "Mechanical": 12}.get(category, 8)
    open_score = 10 if status == "Open" else 5 if status == "Under Review" else 0
    return int(round(min(100, duration_score + location_score + category_score + open_score)))
...
def red_rating_executive_summary(df, production_rate=0.0, value_per_tonne=0.0):
    if df.empty:
        return "No red-rating incidents are available for the selected period."
    work = df.copy()
    hours = pd.to_numeric(work.get("Duration Hours", 0), errors="coerce").fillna(0).sum()
    loc = explode_red_locations(work)
    top_location = "not identified" if loc.empty else loc["Location"].value_counts().index[0]
    cause = "not specified"
    if "Category" in work.columns and not work["Category"].dropna().empty:
        cause = work.groupby("Category")["Duration Hours"].sum().sort_values(ascending=False).index[0]
    open_count = int(work["Status"].isin(["Open", "Under Review"]).sum()) if "Status" in work.columns else 0
    tonnes = hours * production_rate
    cost = tonnes * value_per_tonne
    impact = f", representing approximately {tonnes:,.0f} t at risk" if production_rate > 0 else ""
    if value_per_tonne > 0:
        impact += f" and an estimated value exposure of R {cost:,.0f}"
    return (f"The selected period recorded {len(work)} red-rating incident(s), resulting in {hours:.2f} hours lost{impact}. "
            f"The dominant recorded cause was {cause}, while {top_location} was the most frequently affected location. "
            f"There are currently {open_count} open or under-review incident(s). Priority should be given to recurring hotspots, "
            f"high-severity incidents and overdue corrective actions.")


def save_red_timeline_event(record_id, stage, note, user):
    record = pd.DataFrame([{
        "Record ID": record_id, "Timestamp": datetime.now().isoformat(timespec="seconds"),
        "Stage": stage, "Note": note, "Entered By": user or "Unknown"
    }])
    existing = load_csv(RED_RATING_TIMELINE_FILE)
    save_csv(pd.concat([existing, record], ignore_index=True) if not existing.empty else record, RED_RATING_TIMELINE_FILE)


def save_red_attachments(record_id, uploaded_files):
    saved = []
    if not uploaded_files:
        return saved
    safe_id = re.sub(r"[^A-Za-z0-9_-]", "_", str(record_id))
    folder = RED_RATING_ATTACHMENTS_DIR / safe_id
...
def save_corrective_action_review(record_id, action, review_date, result, recurrence, effectiveness, reviewer):
    record = {
        "Record ID": str(record_id),
        "Corrective Action": str(action).strip(),
        "Review Date": str(review_date),
        "Review Result": str(result).strip(),
        "Recurrence Observed": str(recurrence),
        "Effectiveness Score": int(effectiveness),
        "Reviewed By": reviewer or "Unknown",
        "Timestamp": datetime.now().isoformat(timespec="seconds"),
    }
    save_or_update(RED_RATING_EFFECTIVENESS_FILE, record, ["Record ID"])


def save_lesson_learned(record_id, location, category, lesson, recommendation, user):
    record = {
        "Record ID": str(record_id),
        "Location": str(location).strip(),
        "Category": str(category).strip(),
        "Lesson Learned": str(lesson).strip(),
        "Future Recommendation": str(recommendation).strip(),
        "Entered By": user or "Unknown",
        "Timestamp": datetime.now().isoformat(timespec="seconds"),
    }
    existing = load_csv(RED_RATING_LESSONS_FILE)
    combined = pd.concat([existing, pd.DataFrame([record])], ignore_index=True) if not existing.empty else pd.DataFrame([record])
    save_csv(combined, RED_RATING_LESSONS_FILE)


def response_time_metrics(timeline_df):
    """Calculate elapsed hours from Logged to key investigation stages."""
    if timeline_df.empty or not {"Record ID", "Timestamp", "Stage"}.issubset(timeline_df.columns):
        return pd.DataFrame()
    work = timeline_df.copy()
    work["Timestamp"] = pd.to_datetime(work["Timestamp"], errors="coerce")
    work = work.dropna(subset=["Timestamp"])
    rows = []
    stages = ["Inspection", "Investigation", "Corrective Action", "Verification", "Closed"]
    for record_id, group in work.groupby("Record ID"):
        group = group.sort_values("Timestamp")
        base_rows = group[group["Stage"].astype(str).isin(["Logged", "Red Declared"])]
        base = base_rows["Timestamp"].iloc[0] if not base_rows.empty else group["Timestamp"].iloc[0]
        row = {"Record ID": record_id}
        for stage in stages:
            match = group[group["Stage"].astype(str) == stage]
            row[f"Hours to {stage}"] = ((match["Timestamp"].iloc[0] - base).total_seconds() / 3600) if not match.empty else None
        rows.append(row)
    return pd.DataFrame(rows)


def repeat_offender_summary(red_df):
    if red_df.empty:
        return pd.DataFrame()
    loc = explode_red_locations(red_df)
    if loc.empty:
        return pd.DataFrame()
    loc["Date"] = pd.to_datetime(loc["Date"], errors="coerce")
    loc["Month"] = loc["Date"].dt.to_period("M").astype(str)
    summary = loc.groupby("Location", as_index=False).agg(
        Incidents=("Location", "size"),
        Hours_Lost=("Duration Hours", "sum"),
        Months_Affected=("Month", "nunique"),
        First_Incident=("Date", "min"),
        Latest_Incident=("Date", "max"),
    )
    current_month = loc["Date"].max().to_period("M") if loc["Date"].notna().any() else None
    previous_month = current_month - 1 if current_month is not None else None
    trend_rows = []
    for location in summary["Location"]:
        subset = loc[loc["Location"] == location]
        current_count = int((subset["Date"].dt.to_period("M") == current_month).sum()) if current_month is not None else 0
        previous_count = int((subset["Date"].dt.to_period("M") == previous_month).sum()) if previous_month is not None else 0
        trend = "Worsening" if current_count > previous_count else "Improving" if current_count < previous_count else "Stable"
        trend_rows.append(trend)
    summary["Trend"] = trend_rows
    summary["Repeat Level"] = summary["Incidents"].map(lambda n: "Critical repeat" if n >= 5 else "Repeat hotspot" if n >= 3 else "Monitor" if n == 2 else "Single incident")
    return summary.sort_values(["Incidents", "Hours_Lost"], ascending=False)


def monthly_red_scorecard(red_df, production_rate=0.0, value_per_tonne=0.0):
    if red_df.empty:
        return pd.DataFrame()
    work = red_df.copy()
    ...
    score = work.groupby("Month", as_index=False).agg(
        Red_Ratings=("Record ID", "count"),
        Hours_Lost=("Duration Hours", "sum"),
        Average_Closure_Hours=("Duration Hours", "mean"),
        Open_Incidents=("Status", lambda s: int(pd.Series(s).isin(["Open", "Under Review"]).sum())),
    )
    loc = explode_red_locations(work)
    recurring = {}
    if not loc.empty:
        loc["Month"] = pd.to_datetime(loc["Date"], errors="coerce").dt.to_period("M").astype(str)
        counts = loc.groupby(["Month", "Location"]).size().reset_index(name="Count")
        recurring = counts[counts["Count"] >= 2].groupby("Month")["Location"].nunique().to_dict()
    score["Recurring_Hotspots"] = score["Month"].map(recurring).fillna(0).astype(int)
    score["Estimated_Tonnes_Lost"] = score["Hours_Lost"] * float(production_rate)
    score["Estimated_Value_Exposure_R"] = score["Estimated_Tonnes_Lost"] * float(value_per_tonne)
    score["Performance_Score"] = score.apply(
        lambda r: int(round(max(0, min(100, 100 - min(45, r["Hours_Lost"] * 3) - min(25, r["Red_Ratings"] * 4) - min(20, r["Recurring_Hotspots"] * 8) - min(10, r["Open_Incidents"] * 5))))), axis=1
    )
    return score.sort_values("Month")
...
def red_rating_data_quality(df):
    """Return issue list, readiness score and checked-row count for red-rating records."""
    if df.empty:
        return ["No red-rating records are available."], 0, 0
    issues = []
    required = ["Record ID", "Date", "Start Time", "End Time", "Locations", "Category", "Status"]
    for col in required:
        if col not in df.columns:
            issues.append(f"Missing required column: {col}.")
    work = df.copy()
    rows = len(work)
    if "Record ID" in work.columns:
        dupes = int(work["Record ID"].astype(str).duplicated().sum())
        if dupes:
            issues.append(f"{dupes} duplicate Record ID(s) detected.")
    for idx, row in work.iterrows():
        rid = str(row.get("Record ID", f"row {idx + 2}"))
        if pd.isna(row.get("Date", None)) or str(row.get("Date", "")).strip() == "":
            issues.append(f"{rid}: missing incident date.")
        if not str(row.get("Locations", "")).strip():
            issues.append(f"{rid}: no affected location recorded.")
        if str(row.get("Category", "")).strip() in ["", "Unspecified", "nan"]:
            issues.append(f"{rid}: root cause/category is not specified.")
        if str(row.get("Status", "")).strip() not in ["Open", "Under Review", "Closed"]:
            issues.append(f"{rid}: invalid status '{row.get('Status', '')}'.")
        hours = to_float(row.get("Duration Hours", 0), -1)
        if hours < 0 or hours > 72:
            issues.append(f"{rid}: duration of {hours:.2f} h requires verification.")
        if str(row.get("Status", "")) == "Closed" and not str(row.get("Outcome", "")).strip():
            issues.append(f"{rid}: closed incident has no outcome/reopening condition.")
    # Score penalises unique issues but never below zero.
    readiness = int(max(0, round(100 - min(100, len(issues) * 6))))
    return issues, readiness, rows


def red_rating_escalations(df, timeline_df, effectiveness_df, overdue_hours=4.0):
    """Build a transparent escalation register from current incident records."""
    if df.empty:
        return pd.DataFrame(columns=["Priority", "Record ID", "Reason", "Owner"])
    rows = []
    now = datetime.now()
    work = df.copy()
    for _, row in work.iterrows():
        rid = str(row.get("Record ID", ""))
        status = str(row.get("Status", ""))
        severity = to_int(row.get("Severity Score", red_rating_severity(row)))
        owner = str(row.get("Responsible Person", "Unassigned")) or "Unassigned"
        hours = to_float(row.get("Duration Hours", 0))
        if severity >= 75:
            rows.append({"Priority": "Critical", "Record ID": rid, "Reason": f"Critical severity score ({severity}/100)", "Owner": owner})
        if status in ["Open", "Under Review"] and hours >= overdue_hours:
            rows.append({"Priority": "High", "Record ID": rid, "Reason": f"Restriction remains {status.lower()} after {hours:.2f} h", "Owner": owner})
        stage = str(row.get("Investigation Stage", "Logged"))
        if status != "Closed" and stage in ["Logged", "Inspection"] and hours >= 2:
            rows.append({"Priority": "High", "Record ID": rid, "Reason": f"Investigation has not progressed beyond {stage}", "Owner": owner})
    if not effectiveness_df.empty and {"Record ID", "Review Result"}.issubset(effectiveness_df.columns):
        bad = effectiveness_df[effectiveness_df["Review Result"].astype(str).isin(["Ineffective", "Partly effective"])]
        for _, r in bad.iterrows():
            rows.append({"Priority": "High" if str(r.get("Review Result")) == "Ineffective" else "Moderate",
                         "Record ID": str(r.get("Record ID", "")),
                         "Reason": f"Corrective action assessed as {r.get('Review Result')}",
                         "Owner": str(r.get("Reviewed By", "Unassigned"))})
    out = pd.DataFrame(rows)
    if out.empty:
        return pd.DataFrame(columns=["Priority", "Record ID", "Reason", "Owner"])
    rank = {"Critical": 0, "High": 1, "Moderate": 2, "Low": 3}
    out["_rank"] = out["Priority"].map(rank).fillna(9)
    return out.sort_values(["_rank", "Record ID"]).drop(columns="_rank").drop_duplicates()


def save_red_rating_approval(record_id, workflow_status, user, role, note=""):
    record = {
        "Record ID": str(record_id), "Workflow Status": str(workflow_status),
        "User": user or "Unknown", "Role": role or "Unknown",
        "Note": str(note).strip(), "Timestamp": datetime.now().isoformat(timespec="seconds"),
    }
    existing = load_csv(RED_RATING_APPROVAL_FILE)
    combined = pd.concat([existing, pd.DataFrame([record])], ignore_index=True) if not existing.empty else pd.DataFrame([record])
    save_csv(combined, RED_RATING_APPROVAL_FILE)
    save_red_timeline_event(record_id, f"Workflow: {workflow_status}", note or f"Status set to {workflow_status}", user)


def latest_red_rating_approvals():
    df = load_csv(RED_RATING_APPROVAL_FILE)
    if df.empty or "Record ID" not in df.columns:
        return pd.DataFrame(columns=["Record ID", "Workflow Status", "User", "Role", "Timestamp"])
    if "Timestamp" in df.columns:
        df["_time"] = pd.to_datetime(df["Timestamp"], errors="coerce")
        df = df.sort_values("_time", kind="stable")
    return df.drop_duplicates("Record ID", keep="last").drop(columns=["_time"], errors="ignore")


def build_red_rating_monthly_report(month_label, df, escalations, readiness, issues, production_rate=0.0, value_per_tonne=0.0):
    """Create a Word management report from recorded red-rating data."""
    doc = Document()
    doc.add_heading("GeoIntel Red Rating Management Report", 0)
    doc.add_paragraph(f"Reporting month: {month_label}")
    doc.add_heading("Executive Summary", level=1)
    doc.add_paragraph(red_rating_executive_summary(df, production_rate, value_per_tonne))
    total_hours = pd.to_numeric(df.get("Duration Hours", 0), errors="coerce").fillna(0).sum() if not df.empty else 0
    kpis = {
        "Incidents": len(df), "Hours Lost": f"{total_hours:.2f}",
        "Estimated Tonnes Lost": f"{total_hours * production_rate:,.0f}" if production_rate > 0 else "Not assessed",
        "Estimated Value Exposure": f"R {total_hours * production_rate * value_per_tonne:,.0f}" if production_rate > 0 and value_per_tonne > 0 else "Not assessed",
        "Data Readiness": f"{readiness}/100",
    }
    ...
    doc.add_heading("Data Quality", level=1)
    if issues:
        for item in issues[:25]: doc.add_paragraph(item, style="List Bullet")
    else:
        doc.add_paragraph("All core data-quality checks passed.")
    doc.add_heading("Escalations", level=1)
    if escalations.empty:
        doc.add_paragraph("No active escalations were identified.")
    else:
        for _, r in escalations.iterrows():
            doc.add_paragraph(f"{r.get('Priority')}: {r.get('Record ID')} — {r.get('Reason')} (Owner: {r.get('Owner')})", style="List Bullet")
    doc.add_heading("Incident Register", level=1)
    cols = [c for c in ["Date", "Record ID", "Locations", "Category", "Duration Hours", "Status", "Severity"] if c in df.columns]
...
.rr-hero {background:linear-gradient(135deg,#5f1016,#991b1b 55%,#c2410c);color:white;border-radius:14px;padding:1rem 1.15rem;margin:.15rem 0 .75rem 0;box-shadow:0 8px 22px rgba(127,29,29,.16);}
.rr-hero-title {font-size:1.45rem;font-weight:900;letter-spacing:.01em;}
.rr-hero-sub {font-size:.86rem;opacity:.92;margin-top:.2rem;}
.rr-command {display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.65rem;margin:.55rem 0 .8rem 0;}
.rr-card {background:white;border:1px solid #e5e7eb;border-top:4px solid #b91c1c;border-radius:10px;padding:.7rem .8rem;box-shadow:0 2px 8px rgba(15,23,42,.05);}
.rr-card-label {font-size:.76rem;text-transform:uppercase;letter-spacing:.06em;color:#64748b;font-weight:800;}
.rr-card-value {font-size:1.35rem;color:#111827;font-weight:900;margin-top:.12rem;}
.rr-card-note {font-size:.72rem;color:#64748b;margin-top:.05rem;}
.rr-panel {background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:.8rem .9rem;margin:.4rem 0 .7rem 0;}
.rr-section-title {font-size:1.02rem;font-weight:900;color:#7f1d1d;margin-bottom:.25rem;}
@media (max-width:900px){.rr-command{grid-template-columns:repeat(2,minmax(0,1fr));}}
...
with tab_red_rating:
    ...
    if not red_df.empty:
        red_df["Severity Score"] = red_df.apply(red_rating_severity, axis=1)
        red_df["Severity"] = red_df["Severity Score"].map(severity_label)

    st.markdown(
        '<div class="rr-hero"><div class="rr-hero-title">🚨 Red Rating Management Centre</div>'
        '<div class="rr-hero-sub">Capture restrictions, quantify production impact, investigate repeat hotspots and manage closure assurance.</div></div>',
        unsafe_allow_html=True,
    )

    if red_df.empty:
        active_count = 0; month_hours = 0.0; month_incidents = 0; top_location = "No data"
    else:
        today_rr = datetime.now().date()
        month_start_rr = today_rr.replace(day=1)
        active_count = int(red_df["Status"].astype(str).isin(["Open", "Under Review"]).sum())
        month_mask_rr = red_df["Date"].notna() & (red_df["Date"].dt.date >= month_start_rr) & (red_df["Date"].dt.date <= today_rr)
        month_hours = float(pd.to_numeric(red_df.loc[month_mask_rr, "Duration Hours"], errors="coerce").fillna(0).sum())
        month_incidents = int(month_mask_rr.sum())
        exploded_rr = explode_red_rating_locations(red_df)
        if exploded_rr.empty:
            top_location = "No data"
        else:
            top_location = str(exploded_rr.groupby("Location").size().sort_values(ascending=False).index[0])

    st.markdown(
        f'<div class="rr-command">'
        f'<div class="rr-card"><div class="rr-card-label">Active restrictions</div><div class="rr-card-value">{active_count}</div><div class="rr-card-note">Open or under review</div></div>'
        f'<div class="rr-card"><div class="rr-card-label">Month incidents</div><div class="rr-card-value">{month_incidents}</div><div class="rr-card-note">Current calendar month</div></div>'
        f'<div class="rr-card"><div class="rr-card-label">Month hours lost</div><div class="rr-card-value">{month_hours:.2f} h</div><div class="rr-card-note">Calculated from incident durations</div></div>'
        f'<div class="rr-card"><div class="rr-card-label">Leading hotspot</div><div class="rr-card-value">{top_location}</div><div class="rr-card-note">Most frequently affected location</div></div>'
        f'</div>', unsafe_allow_html=True
    )

    dashboard_tab, capture_tab, excel_tab, intelligence_tab, control_tab, history_tab = st.tabs([
        "📊 Command Centre", "➕ Capture", "📥 Excel Import", "🧠 Investigation", "✅ Control & Reports", "🗂️ Register"
    ])

    with capture_tab:
        st.markdown('<div class="rr-panel"><div class="rr-section-title">New Red-Rating Incident</div>Capture the restriction once. GeoIntel calculates duration, saves the investigation stage and feeds every dashboard automatically.</div>', unsafe_allow_html=True)
        with st.form("red_rating_capture_form_v2", clear_on_submit=True):
            ...
            investigation_stage = st.selectbox("Investigation stage", ["Logged", "Inspection", "Investigation", "Corrective Action", "Verification", "Closed"], key="rr2_investigation_stage")
        ...
        if submit_red:
            ...
            if not final_locations:
                ...
                record = {
                    ...
                    "Status": status, "Investigation Stage": investigation_stage, "Entered By": current_user if 'current_user' in globals() else "Unknown",
                    "Timestamp": datetime.now().isoformat(timespec="seconds"),
                }
    ...
    with dashboard_tab:
        if red_df.empty:
            ...
            st.markdown('<div class="rr-panel"><div class="rr-section-title">Operational Command Centre</div>Review current exposure, dominant causes, repeat locations and unresolved restrictions from one screen.</div>', unsafe_allow_html=True)
            s1, s2 = st.columns(2)
    ...
    with intelligence_tab:
        if red_df.empty:
            st.info("No incidents are available for intelligence analysis.")
        else:
            st.markdown('<div class="section-bar">RED RATING DECISION INTELLIGENCE</div>', unsafe_allow_html=True)
            i1, i2 = st.columns(2)
            intel_rate = i1.number_input("Production rate for summary (t/h)", min_value=0.0, value=500.0, step=10.0, key="rr_intel_rate")
            intel_value = i2.number_input("Value per tonne for summary (R)", min_value=0.0, value=0.0, step=1.0, key="rr_intel_value")
            summary_text = red_rating_executive_summary(red_df, intel_rate, intel_value)
            st.subheader("Executive Summary Generator")
            st.text_area("Generated management summary", value=summary_text, height=150, key="rr_exec_summary")
            st.download_button("Download executive summary", summary_text.encode("utf-8"), "red_rating_executive_summary.txt", "text/plain", use_container_width=True)

            st.subheader("Severity Index")
            severity_cols = [c for c in ["Record ID", "Date", "Locations", "Category", "Duration Hours", "Severity Score", "Severity", "Status"] if c in red_df.columns]
            st.dataframe(red_df.sort_values("Severity Score", ascending=False)[severity_cols], width="stretch", hide_index=True)

            record_options = red_df["Record ID"].dropna().astype(str).tolist()
            selected_record = st.selectbox("Select incident", record_options, key="rr_intel_record")
            selected_incident = red_df[red_df["Record ID"].astype(str) == selected_record].iloc[-1]

            t1, t2 = st.columns(2)
            with t1:
                st.subheader("Investigation Tracker")
                current_stage = str(selected_incident.get("Investigation Stage", "Logged"))
                stages = ["Logged", "Inspection", "Investigation", "Corrective Action", "Verification", "Closed"]
                stage_index = stages.index(current_stage) if current_stage in stages else 0
                new_stage = st.selectbox("Current stage", stages, index=stage_index, key="rr_update_stage")
                stage_note = st.text_area("Stage note / finding", key="rr_stage_note")
                if st.button("Update investigation stage", type="primary", key="rr_update_stage_button"):
                    stored = load_csv(RED_RATING_FILE)
                    mask = stored["Record ID"].astype(str) == selected_record
                    stored.loc[mask, "Investigation Stage"] = new_stage
                    if new_stage == "Closed":
                        stored.loc[mask, "Status"] = "Closed"
                    save_csv(stored, RED_RATING_FILE)
                    save_red_timeline_event(selected_record, new_stage, stage_note, current_user if 'current_user' in globals() else "Unknown")
                    st.success("Investigation stage updated.")
                    st.rerun()
            with t2:
                st.subheader("Attachments")
                attachments = st.file_uploader("Attach photos, PDFs, spreadsheets or documents", accept_multiple_files=True, key="rr_attachments")
                if st.button("Save attachments", key="rr_save_attachments"):
                    saved_paths = save_red_attachments(selected_record, attachments)
                    if saved_paths:
                        save_red_timeline_event(selected_record, "Attachment", f"Saved {len(saved_paths)} attachment(s)", current_user if 'current_user' in globals() else "Unknown")
                        st.success(f"Saved {len(saved_paths)} attachment(s).")
                    else:
                        st.warning("Choose at least one file first.")
                safe_id = re.sub(r"[^A-Za-z0-9_-]", "_", selected_record)
                folder = RED_RATING_ATTACHMENTS_DIR / safe_id
                if folder.exists():
                    st.caption("Saved attachments")
                    for path in sorted(folder.iterdir()):
                        if path.is_file():
                            st.download_button(path.name, path.read_bytes(), file_name=path.name, key=f"dl_{safe_id}_{path.name}")

            st.subheader("Incident Timeline")
            timeline = load_csv(RED_RATING_TIMELINE_FILE)
            if not timeline.empty and "Record ID" in timeline.columns:
                timeline = timeline[timeline["Record ID"].astype(str) == selected_record].copy()
            if timeline.empty:
                st.info("No timeline events have been recorded for this incident yet.")
            else:
                st.dataframe(timeline.sort_values("Timestamp")[["Timestamp", "Stage", "Note", "Entered By"]], width="stretch", hide_index=True)

            st.markdown('<div class="section-bar">CONTINUOUS IMPROVEMENT</div>', unsafe_allow_html=True)
            ci1, ci2 = st.columns(2)
            with ci1:
                st.subheader("Corrective Action Effectiveness")
                existing_effectiveness = load_csv(RED_RATING_EFFECTIVENESS_FILE)
                action_default = str(selected_incident.get("Action / Comments", ""))
                effectiveness_action = st.text_area("Corrective action assessed", value=action_default, key="rr_effectiveness_action")
                review_date = st.date_input("Review date", value=datetime.now().date(), key="rr_effectiveness_date")
                review_result = st.selectbox("Review result", ["Effective", "Partly effective", "Ineffective", "Too early to assess"], key="rr_effectiveness_result")
                recurrence = st.selectbox("Recurrence observed after action?", ["No", "Yes", "Unknown"], key="rr_recurrence")
                effectiveness_score = st.slider("Effectiveness score", 0, 100, 80, key="rr_effectiveness_score")
                if st.button("Save effectiveness review", type="primary", key="rr_save_effectiveness"):
                    save_corrective_action_review(selected_record, effectiveness_action, review_date, review_result, recurrence, effectiveness_score, current_user if 'current_user' in globals() else "Unknown")
                    save_red_timeline_event(selected_record, "Effectiveness Review", f"{review_result}; score {effectiveness_score}/100; recurrence {recurrence}", current_user if 'current_user' in globals() else "Unknown")
                    st.success("Corrective-action effectiveness saved.")
                if not existing_effectiveness.empty:
                    selected_review = existing_effectiveness[existing_effectiveness["Record ID"].astype(str) == selected_record]
                    if not selected_review.empty:
                        st.dataframe(selected_review.tail(1), width="stretch", hide_index=True)

            with ci2:
                st.subheader("Lessons Learned Library")
                primary_location = str(selected_incident.get("Locations", "")).split(",")[0].strip()
                lesson = st.text_area("What did we learn?", key="rr_lesson")
                future_recommendation = st.text_area("Recommendation for future incidents", key="rr_future_recommendation")
                if st.button("Save lesson learned", key="rr_save_lesson"):
                    if lesson.strip():
                        save_lesson_learned(selected_record, primary_location, selected_incident.get("Category", ""), lesson, future_recommendation, current_user if 'current_user' in globals() else "Unknown")
                        st.success("Lesson added to the library.")
                    else:
                        st.warning("Enter a lesson before saving.")
                lessons = load_csv(RED_RATING_LESSONS_FILE)
                lesson_search = st.text_input("Search lessons", key="rr_lesson_search")
                if not lessons.empty:
                    shown_lessons = lessons.copy()
                    if lesson_search.strip():
                        mask = shown_lessons.astype(str).apply(lambda c: c.str.contains(lesson_search.strip(), case=False, na=False)).any(axis=1)
                        shown_lessons = shown_lessons[mask]
                    st.dataframe(shown_lessons.sort_values("Timestamp", ascending=False), width="stretch", hide_index=True)

            st.subheader("Response Time KPI")
            all_timeline = load_csv(RED_RATING_TIMELINE_FILE)
            response = response_time_metrics(all_timeline)
            if response.empty:
                st.info("Response-time KPIs will appear after investigation stages are timestamped.")
            else:
                metric_cols = [c for c in response.columns if c.startswith("Hours to ")]
                averages = response[metric_cols].apply(pd.to_numeric, errors="coerce").mean()
                rcols = st.columns(min(5, len(metric_cols)))
                for index, col in enumerate(metric_cols[:5]):
                    value = averages.get(col)
                    rcols[index].metric(col.replace("Hours to ", "Avg. to "), "Waiting" if pd.isna(value) else f"{value:.2f} h")
                st.dataframe(response, width="stretch", hide_index=True)

            st.subheader("Repeat Offender Dashboard")
            repeats = repeat_offender_summary(red_df)
            if repeats.empty:
                st.info("No location history is available.")
            else:
                st.dataframe(repeats.head(20), width="stretch", hide_index=True)
                st.plotly_chart(px.bar(repeats.head(10), x="Location", y="Incidents", color="Trend", hover_data=["Hours_Lost", "Months_Affected", "Repeat Level"], title="Top repeat-offender locations"), width="stretch")

            st.subheader("Monthly Performance Scorecard")
            sc1, sc2 = st.columns(2)
            score_rate = sc1.number_input("Scorecard production rate (t/h)", min_value=0.0, value=500.0, step=10.0, key="rr_score_rate")
            score_value = sc2.number_input("Scorecard value per tonne (R)", min_value=0.0, value=0.0, step=1.0, key="rr_score_value")
            scorecard = monthly_red_scorecard(red_df, score_rate, score_value)
            if scorecard.empty:
                st.info("No monthly scorecard data is available.")
            else:
                st.dataframe(scorecard, width="stretch", hide_index=True)
                st.plotly_chart(px.line(scorecard, x="Month", y="Performance_Score", markers=True, range_y=[0, 100], title="Monthly red-rating performance score"), width="stretch")

    with control_tab:
        st.markdown('<div class="rr-panel"><div class="rr-section-title">Control, Assurance & Reporting</div>Validate records, manage escalation and approval, compare incidents and issue the monthly report.</div>', unsafe_allow_html=True)
        quality_issues, readiness_score, checked_rows = red_rating_data_quality(red_df)
        q1, q2, q3 = st.columns(3)
        q1.metric("Data readiness", f"{readiness_score}/100")
        q2.metric("Records checked", checked_rows)
        q3.metric("Quality issues", len(quality_issues))
        if quality_issues:
            with st.expander("Data-quality issues", expanded=readiness_score < 80):
                for issue in quality_issues:
                    st.warning(issue)
        else:
            st.success("All core data-quality checks passed.")

        timeline_all = load_csv(RED_RATING_TIMELINE_FILE)
        effectiveness_all = load_csv(RED_RATING_EFFECTIVENESS_FILE)
        overdue_limit = st.number_input("Escalate open restrictions after (hours)", min_value=0.5, value=4.0, step=0.5, key="rr_escalate_hours")
        escalation_df = red_rating_escalations(red_df, timeline_all, effectiveness_all, overdue_limit)
        st.subheader("Escalation Centre")
        if escalation_df.empty:
            st.success("No active escalation triggers were identified.")
        else:
            st.dataframe(escalation_df, width="stretch", hide_index=True)

        st.subheader("Incident Approval Workflow")
        if red_df.empty:
            st.info("No incidents are available for review.")
        else:
            approval_ids = red_df["Record ID"].astype(str).tolist()
            approval_id = st.selectbox("Incident for workflow", approval_ids, key="rr_approval_id")
            approvals_latest = latest_red_rating_approvals()
            current_workflow = "Draft"
            if not approvals_latest.empty:
                current_rows = approvals_latest[approvals_latest["Record ID"].astype(str) == approval_id]
                if not current_rows.empty: current_workflow = str(current_rows.iloc[-1].get("Workflow Status", "Draft"))
            st.info(f"Current workflow status: **{current_workflow}**")
            wf1, wf2 = st.columns([1, 2])
            workflow_status = wf1.selectbox("Set workflow status", ["Draft", "Reviewed", "Approved", "Closed"], index=["Draft", "Reviewed", "Approved", "Closed"].index(current_workflow) if current_workflow in ["Draft", "Reviewed", "Approved", "Closed"] else 0, key="rr_workflow_status")
            workflow_note = wf2.text_input("Review / approval note", key="rr_workflow_note")
            role_value = current_role if 'current_role' in globals() else "Unknown"
            user_value = current_user if 'current_user' in globals() else "Unknown"
            can_review = role_value in ["Strata Control Officer", "Geotechnical Specialist", "Geotechnical Superintendent", "Manager"]
            can_approve = role_value in ["Geotechnical Superintendent", "Manager"]
            permitted = workflow_status in ["Draft"] or (workflow_status == "Reviewed" and can_review) or (workflow_status in ["Approved", "Closed"] and can_approve)
            if st.button("Save workflow status", type="primary", disabled=not permitted, key="rr_save_workflow"):
                save_red_rating_approval(approval_id, workflow_status, user_value, role_value, workflow_note)
                st.success("Workflow status saved and added to the incident timeline.")
                st.rerun()
            if not permitted:
                st.caption("Your selected role does not have permission for this workflow status.")
            if not approvals_latest.empty:
                st.dataframe(approvals_latest.sort_values("Timestamp", ascending=False), width="stretch", hide_index=True)

        st.subheader("Incident Comparison")
        if len(red_df) < 2:
            st.info("At least two incidents are needed for comparison.")
        else:
            ids = red_df["Record ID"].astype(str).tolist()
            c1, c2 = st.columns(2)
            id_a = c1.selectbox("Incident A", ids, index=0, key="rr_compare_a")
            id_b = c2.selectbox("Incident B", ids, index=1, key="rr_compare_b")
            a = red_df[red_df["Record ID"].astype(str) == id_a].iloc[-1]
            b = red_df[red_df["Record ID"].astype(str) == id_b].iloc[-1]
            comparison_fields = ["Date", "Locations", "Category", "Duration Hours", "Status", "Investigation Stage", "Severity Score", "Severity", "Responsible Person", "Action / Comments", "Outcome"]
            comparison = pd.DataFrame({"Metric": comparison_fields, f"Incident {id_a}": [a.get(x, "") for x in comparison_fields], f"Incident {id_b}": [b.get(x, "") for x in comparison_fields]})
            st.dataframe(comparison, width="stretch", hide_index=True)

        st.subheader("Monthly Management Report")
        if red_df.empty or red_df["Date"].dropna().empty:
            st.info("No dated incident records are available for reporting.")
        else:
            report_work = red_df.copy()
            report_work["Report Month"] = report_work["Date"].dt.to_period("M").astype(str)
            month_options = sorted(report_work["Report Month"].dropna().unique(), reverse=True)
            report_month = st.selectbox("Reporting month", month_options, key="rr_report_month")
            report_rate = st.number_input("Production rate for report (t/h)", min_value=0.0, value=500.0, step=10.0, key="rr_report_rate")
            report_value = st.number_input("Estimated value per tonne for report (R)", min_value=0.0, value=0.0, step=1.0, key="rr_report_value")
            month_df = report_work[report_work["Report Month"] == report_month].drop(columns="Report Month")
            month_issues, month_readiness, _ = red_rating_data_quality(month_df)
            month_escalations = red_rating_escalations(month_df, timeline_all, effectiveness_all, overdue_limit)
            report_buffer = build_red_rating_monthly_report(report_month, month_df, month_escalations, month_readiness, month_issues, report_rate, report_value)
            st.download_button("Download monthly Word report", report_buffer, file_name=f"GeoIntel_Red_Rating_Report_{report_month}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", type="primary", use_container_width=True)
