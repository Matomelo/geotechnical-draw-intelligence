...
with tab_red_rating:
    ...
    with capture_tab:
        with st.form("red_rating_capture_form", clear_on_submit=True):
            ...
            c1, c2, c3 = st.columns(3)
            category = c1.selectbox("Category", ["Seismic red rating", "Re-entry restriction", "Inspection delay", "System / communication delay", "Other"], key="rr_category")
    ...
    with dashboard_tab:
        summary = red_rating_summary(red_df)
        d1, d2, d3, d4 = st.columns(4)
        d1.metric("Incidents", summary["incidents"])
        d2.metric("Total time lost", f"{summary['hours']:.2f} h")
        d3.metric("Open / under review", summary["open"])
        d4.metric("Most affected location", summary["top_location"])

        if red_df.empty:
            st.info("No red-rating records have been saved yet.")
        else:
            work = red_df.copy()
            work["Date"] = pd.to_datetime(work["Date"], errors="coerce")
            work["Duration Hours"] = pd.to_numeric(work["Duration Hours"], errors="coerce").fillna(0.0)
            daily = work.dropna(subset=["Date"]).groupby("Date", as_index=False)["Duration Hours"].sum()
            if not daily.empty:
                fig = px.bar(daily, x="Date", y="Duration Hours", title="Red-rating time lost by date")
                fig.update_layout(height=360, margin=dict(l=10, r=10, t=45, b=10))
                st.plotly_chart(fig, width="stretch")

            if "Locations" in work.columns:
                loc = work[["Locations", "Duration Hours"]].dropna()
                loc = loc.assign(Location=loc["Locations"].astype(str).str.split(",")).explode("Location")
                loc["Location"] = loc["Location"].str.strip()
                loc_summary = loc.groupby("Location", as_index=False)["Duration Hours"].sum().sort_values("Duration Hours", ascending=False)
