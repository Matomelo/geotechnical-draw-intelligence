...
DEFAULT_MAP_FILE = Path("lift2east_polygon_map.png")
...
def polygon_health_score(ep, et, events, potency, ep_threshold, et_threshold):
    """Return a 0-100 polygon health score from the current matrix response.

    Version 1 uses E/P, E/T, event frequency and data completeness.
    Production-target achievement and time trends can be added once those fields
    are available consistently for every polygon.
    """
    if potency <= 0 or et is None:
        return 0
    return draw_compliance_score(
        ep=ep,
        et=et,
        events=events,
        potency=potency,
...
def polygon_map_figure(results_df, map_image):
    """Colour the six polygon areas without duplicate titles or legends."""
    image = Image.open(map_image) if not isinstance(map_image, Image.Image) else map_image

    # The saved PNG contains an old Plotly title/legend strip at the top.
    # Crop that strip before displaying the mine layout.
    source_width, source_height = image.size
    crop_top = 80
    image = image.crop((0, crop_top, source_width, source_height))
    ...
    # Coordinates are proportional to the supplied Lift II East drawing.
    boxes = {
        "D": (0.401, 0.403, 0.491, 0.526),
        "A": (0.491, 0.403, 0.583, 0.526),
        "E": (0.401, 0.526, 0.491, 0.660),
        "B": (0.491, 0.526, 0.583, 0.660),
        "F": (0.401, 0.660, 0.491, 0.787),
        "C": (0.491, 0.660, 0.583, 0.787),
    }
    ...
    for poly, (x0r, y0r, x1r, y1r) in boxes.items():
        row = rows.get(poly, {})
        ...
        x0, x1 = x0r * source_width, x1r * source_width
        # Convert the original-image polygon coordinates to the cropped image.
        y0 = max(0, y0r * source_height - crop_top)
        y1 = max(0, y1r * source_height - crop_top)
        ...
        # Invisible hover target. No text is drawn because the source map already labels A-F.
        fig.add_trace(
            go.Scatter(
                x=[(x0 + x1) / 2],
                y=[(y0 + y1) / 2],
                mode="markers",
                marker=dict(size=95, opacity=0),
                hovertext=[hover],
                hoverinfo="text",
                showlegend=False,
            )
    ...
    # Keep the map clean: the summary cards already explain the status colours.
...
[data-testid="stMetric"] {background: #f7f9fc; border: 1px solid #e2e8f0; padding: 0.7rem 0.8rem; border-radius: 10px;}
.gdi-title {font-size: 2rem; font-weight: 800; margin-bottom: 0.1rem;}
.gdi-subtitle {color: #64748b; margin-bottom: 0.8rem;}
.status-badge {display:inline-block; padding:0.42rem 0.8rem; border-radius:999px; color:white; font-weight:800; margin:0.2rem 0 0.8rem 0;}
.panel {border:1px solid #e2e8f0; border-radius:14px; padding:1rem; background:#ffffff;}
.small-note {color:#64748b; font-size:0.9rem;}
</style>
<div class="gdi-title">⛏️ GDI — Geotechnical Draw Intelligence</div>
<div class="gdi-subtitle">Lift II East polygon draw monitoring and seismic intelligence</div>
...
with tab_polygon:
    ...
    with top2:
        ...
        st.caption("Drawpoint tonnes → Drawbell totals → Polygon tonnes. Energy and potency remain polygon-level values.")
    ...
    if not tonnes_df.empty:
        ...
        unrecognised = template[template["Drawbell"] == ""]
        if not unrecognised.empty:
            st.warning(
                f"{len(unrecognised)} drawpoint name(s) do not match T<number><E/W><two digits>. "
                "Correct those names before relying on the totals."
            )
        ...
        with st.expander("Drawbell → polygon master mapping", expanded=bool(mapping_df.empty)):
            st.caption("Assign the physical polygon once per drawbell. Both drawpoints remain together automatically.")
            edited_bell_mapping = st.data_editor(
                ...
                column_config={
                    "Polygon": st.column_config.SelectboxColumn(
                        "Polygon", options=["", "A", "B", "C", "D", "E", "F"], required=False
                    ),
                },
            ...
            if st.button("Save Master Mapping", key="save_polygon_mapping"):
                ...
                st.success("Master drawpoint → drawbell → polygon mapping saved.")
            with st.expander("Check automatic drawpoint pairing"):
                st.dataframe(edited_mapping, width="stretch", hide_index=True)
        ...
        if not missing.empty:
            st.warning(f"{missing['Drawpoint'].nunique()} drawpoint(s) are not assigned to a polygon.")
    else:
        st.info("Upload the production spreadsheet to calculate polygon tonnes. The dashboard remains available for seismic input.")

    st.markdown("### Polygon seismic input")
    ...
    seismic_input = st.data_editor(
        ...
        hide_index=True,
        disabled=["Polygon"],
        key="polygon_seismic_editor",
    )

    results = seismic_input.merge(polygon_totals, on="Polygon", how="left")
    results["Polygon Tonnes"] = pd.to_numeric(results["Polygon Tonnes"], errors="coerce").fillna(0.0)
    results["Energy J"] = pd.to_numeric(results["Energy J"], errors="coerce").fillna(0.0)
    results["Potency m3"] = pd.to_numeric(results["Potency m3"], errors="coerce").fillna(0.0)
    results["E/P"] = results.apply(
        lambda r: r["Energy J"] / r["Potency m3"] if r["Potency m3"] > 0 else 0.0,
        axis=1,
    )
    results["E/T"] = results.apply(
        lambda r: r["Energy J"] / r["Polygon Tonnes"] if r["Polygon Tonnes"] > 0 else None,
        axis=1,
    )
    results["Status"] = results.apply(
        lambda r: get_status(r["E/P"], r["E/T"], ep_threshold, et_threshold)
        if r["Potency m3"] > 0 else "Waiting for seismic data",
        axis=1,
    )
    ...
    results["Health Rating"] = results["Health Score"].map(health_score_label)
    results["Map Rating"] = results["Status"].map(
        lambda s: "Green" if s == "Efficient Draw"
        else "Red" if s == "High-Risk Draw"
        else "Amber" if s in ["Brittle but Efficient", "Ductile but Costly"]
        else "Grey"
    )
    ...
    with left:
        st.markdown("### Lift II East status map")
        st.markdown("""<style>.js-plotly-plot .gtitle,.js-plotly-plot .legend{display:none!important;}</style>""", unsafe_allow_html=True)
        if DEFAULT_MAP_FILE.exists():
            st.plotly_chart(
                polygon_map_figure(results, DEFAULT_MAP_FILE),
                width="stretch",
                key="polygon_status_map_v33",
                config={
            ...
            st.error("The built-in map is missing. Keep lift2east_polygon_map.png in the same folder as this app.")

    with right:
        st.markdown("### Polygon information")
        bcols = st.columns(3)
        for index, poly in enumerate("ABCDEF"):
            if bcols[index % 3].button(poly, key=f"select_poly_{poly}", use_container_width=True):
        ...
        st.markdown(
            f'<span class="status-badge" style="background:{badge_colour}">{selected["Status"]}</span>',
            unsafe_allow_html=True,
        )
        ...
        health_score = int(selected["Health Score"])
        st.markdown("#### Polygon Health Score")
        hs1, hs2 = st.columns([1, 1.4])
        hs1.metric("Health", f"{health_score}/100")
        hs2.metric("Condition", selected["Health Rating"])
        st.progress(health_score / 100 if health_score > 0 else 0.0)
        st.caption("Based on the current E/P–E/T matrix position, event frequency and data completeness.")
        ...
        if selected["Potency m3"] > 0 and et_display is not None and not pd.isna(et_display):
            ...
            st.plotly_chart(
                ...
                width="stretch",
                key=f"polygon_matrix_{selected_poly}",
            )
        else:
            st.info("Enter polygon potency and upload tonnes to display its matrix position.")

    st.markdown("### Polygon Summary")
    summary_df = results[[
        "Polygon", "Map Rating", "Polygon Tonnes", "E/P", "E/T",
        "Health Score", "Health Rating", "Status"
    ]].copy()
    summary_df = summary_df.rename(columns={"Map Rating": "Rating"})
    st.dataframe(summary_df, width="stretch", hide_index=True)

    st.markdown(f"### Drawbells inside Polygon {st.session_state.selected_polygon}")
    if not drawbell_totals.empty:
        ...
        with d1:
            st.dataframe(poly_drawbells, width="stretch", hide_index=True)
        with d2:
            st.dataframe(poly_points, width="stretch", hide_index=True)
    else:
        st.caption("Drawbell and drawpoint detail will appear after a production spreadsheet is uploaded and mapped.")

    with st.expander("All polygon results"):
        st.dataframe(
            results[["Polygon", "Events", "Energy J", "Potency m3", "Polygon Tonnes", "E/P", "E/T", "Health Score", "Health Rating", "Status", "Map Rating"]],
            width="stretch",
            hide_index=True,
        )

    if st.button("Save Polygon Results", type="primary", key="save_polygon_results"):
