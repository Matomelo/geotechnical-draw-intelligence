...
def combined_risk_index(row):
    """Combine draw, seismic, production and convergence into a 0-100 risk index."""
    score = 0.0
    status = str(row.get("Status", ""))
    score += {
        "Efficient Draw": 5,
        "Brittle but Efficient": 35,
        "Ductile but Costly": 40,
        "High-Risk Draw": 65,
        "Waiting for tonnes": 25,
        "Waiting for seismic data": 20,
    }.get(status, 20)

    health = to_float(row.get("Health Score", 0))
    score += max(0, 25 - min(25, health * 0.25))
    ...
    score += {
        "Combined geotechnical concern": 25,
        "High convergence": 18,
        "Convergence watch": 10,
        "Stable convergence": 0,
        "No convergence data": 4,
    }.get(conv, 4)
...
current_role = st.sidebar.selectbox("Role", ["Technician", "Engineer", "Manager"], index=["Technician", "Engineer", "Manager"].index(st.session_state.get("gdi_role", "Technician")))
...
with tab_polygon:
    ...
    results["Combined Risk Index"] = results.apply(combined_risk_index, axis=1)
    ...
    summary_cols = ["Polygon", "Status", "Combined Risk", "Combined Risk Index", "Health Score", "Polygon Tonnes", "Achievement %", "E/P", "E/T", "Events", "Closure mm", "Rate mm/day", "Convergence Risk", "Neighbour Influence"]
    ...
    if ap2.button("Mark Reviewed", disabled=is_locked or current_role not in ["Engineer", "Manager"], use_container_width=True):
        save_approval_status(polygon_date, "Reviewed", current_user, current_role); st.rerun()
    if ap3.button("Approve & Lock", disabled=is_locked or current_role != "Manager", use_container_width=True):
        save_approval_status(polygon_date, "Approved", current_user, current_role); st.rerun()
    if is_locked and current_role == "Manager" and st.button("Reopen Approved Record", use_container_width=True):
        save_approval_status(polygon_date, "Reviewed", current_user, current_role); st.rerun()
