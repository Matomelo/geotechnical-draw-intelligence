...
def polygon_map_figure(results_df, map_image):
    ...
    fig = go.Figure()
    fig.add_layout_image(dict(source=image, x=0, y=0, sizex=width, sizey=height, xref="x", yref="y", sizing="stretch", layer="below"))

    # Coordinates are proportional to the supplied Lift II East polygon drawing.
    boxes = {
    ...
    rows = {str(r["Polygon"]).upper(): r for _, r in results_df.iterrows()} if not results_df.empty else {}
    for poly, (x0r, y0r, x1r, y1r) in boxes.items():
        ...
        x0, x1 = x0r * width, x1r * width
        y0, y1 = y0r * height, y1r * height
        fig.add_shape(type="rect", x0=x0, x1=x1, y0=y0, y1=y1, fillcolor=colour, opacity=0.35, line=dict(color=colour, width=3))
        hover = f"Polygon {poly}<br>Status: {status}"
        if hasattr(row, "get"):
            hover += f"<br>Tonnes: {to_float(row.get('Polygon Tonnes', 0)):,.0f}<br>E/P: {to_float(row.get('E/P', 0)):,.2f}"
            etv = row.get('E/T', None)
            hover += f"<br>E/T: {to_float(etv):,.2f}" if etv not in [None, ""] and not pd.isna(etv) else "<br>E/T: Waiting"
        fig.add_trace(go.Scatter(x=[(x0+x1)/2], y=[(y0+y1)/2], mode="markers+text", marker=dict(size=80, opacity=0), text=[poly], textfont=dict(size=34, color="black"), hovertext=[hover], hoverinfo="text", showlegend=False))

    fig.update_xaxes(range=[0, width], visible=False)
    fig.update_yaxes(range=[height, 0], visible=False, scaleanchor="x")
    fig.update_layout(height=780, margin=dict(l=0, r=0, t=40, b=0), title="Lift II East Polygon Draw Status")
    return fig
...
with tab_polygon:
    st.header("Lift II East Polygon Draw Map")
    st.write("Energy and potency are entered per polygon. Tonnes are imported per drawpoint, summed to drawbell level, and then summed to polygon level.")

    polygon_date = st.date_input("Reporting Date", value=datetime.now().date(), key="polygon_date")

    st.subheader("1. Upload drawpoint tonnes")
    tonnes_upload = st.file_uploader("Upload production spreadsheet with drawpoint tonnes", type=["xlsx", "xls", "csv"], key="polygon_tonnes_upload")
    ...
    if not tonnes_df.empty:
        st.dataframe(tonnes_df.head(20), width="stretch")
        p1, p2 = st.columns(2)
        drawpoint_col = p1.selectbox("Select drawpoint column", tonnes_df.columns, key="polygon_drawpoint_col")
        tonnes_col = p2.selectbox("Select tonnes column", tonnes_df.columns, key="polygon_tonnes_col")

        template = build_mapping_template(tonnes_df[drawpoint_col])
        unrecognised = template[template["Drawbell"] == ""]
        if not unrecognised.empty:
            st.warning(
                f"{len(unrecognised)} drawpoint name(s) do not match the expected T<number><E/W><two digits> format. "
                "They must be corrected before they can be grouped into drawbells."
            )

        st.subheader("2. Confirm drawbell → polygon mapping")
        st.caption(
            "The app now calculates each drawbell from the drawpoint name. "
            "Assign the polygon once per drawbell; both drawpoints are then kept together automatically."
        )
        ...
        with st.expander("Check the automatic drawpoint → drawbell pairing"):
            st.dataframe(edited_mapping, width="stretch", hide_index=True)

        if st.button("Save Drawbell Mapping", key="save_polygon_mapping"):
            save_csv(edited_mapping, DRAWPOINT_MAPPING_FILE)
            st.success("Drawbell polygon mapping saved and applied to both drawpoints in every drawbell.")

        merged_tonnes, drawbell_totals, polygon_totals = aggregate_polygon_tonnes(tonnes_df, drawpoint_col, tonnes_col, edited_mapping)
        missing = merged_tonnes[merged_tonnes["Polygon"].isna() | (merged_tonnes["Polygon"].astype(str).str.strip() == "")]
        if not missing.empty:
            st.warning(f"{missing['Drawpoint'].nunique()} drawpoint(s) are not assigned to a polygon yet.")

        c1, c2 = st.columns(2)
        with c1:
            st.markdown("**Drawbell totals**")
            st.dataframe(drawbell_totals, width="stretch")
        with c2:
            st.markdown("**Polygon tonnes**")
            st.dataframe(polygon_totals, width="stretch")
    else:
        st.info("Upload the production spreadsheet to calculate drawbell and polygon tonnes.")

    st.subheader("3. Enter polygon seismic values")
    ...
    seismic_input = st.data_editor(seismic_seed, width="stretch", hide_index=True, key="polygon_seismic_editor")
    ...
    results["Status"] = results.apply(lambda r: get_status(r["E/P"], r["E/T"], ep_threshold, et_threshold) if r["Potency m3"] > 0 else "Waiting for seismic data", axis=1)
    results["Map Rating"] = results["Status"].map(lambda s: "Green" if s == "Efficient Draw" else "Red" if s == "High-Risk Draw" else "Amber" if s in ["Brittle but Efficient", "Ductile but Costly"] else "Grey")

    st.subheader("4. Polygon results")
    st.dataframe(results[["Polygon", "Events", "Energy J", "Potency m3", "Polygon Tonnes", "E/P", "E/T", "Status", "Map Rating"]], width="stretch", hide_index=True)

    map_upload = st.file_uploader("Optional: upload the Lift II East polygon map image", type=["png", "jpg", "jpeg"], key="polygon_map_upload")
    map_source = map_upload if map_upload is not None else DEFAULT_MAP_FILE
    if (hasattr(map_source, "read")) or Path(map_source).exists():
        st.plotly_chart(polygon_map_figure(results, map_source), width="stretch")
    else:
        st.warning("Place lift2east_polygon_map.png in the same folder as app.py, or upload the map image above.")

    selected_poly = st.selectbox("Inspect polygon", list("ABCDEF"), key="inspect_polygon")
    selected = results[results["Polygon"] == selected_poly].iloc[0]
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Polygon Tonnes", f"{selected['Polygon Tonnes']:,.0f}")
    m2.metric("E/P", f"{selected['E/P']:,.2f}")
    m3.metric("E/T", f"{selected['E/T']:,.2f}" if selected['E/T'] is not None and not pd.isna(selected['E/T']) else "Waiting")
    m4.metric("Status", selected["Status"])

    if not drawbell_totals.empty:
        st.markdown(f"**Drawbells and drawpoints contributing to Polygon {selected_poly}**")
        poly_drawbells = drawbell_totals[drawbell_totals["Polygon"] == selected_poly]
        st.dataframe(poly_drawbells, width="stretch", hide_index=True)
        poly_points = merged_tonnes[merged_tonnes["Polygon"] == selected_poly][["Drawpoint", "Drawbell", "Tonnes"]]
        st.dataframe(poly_points, width="stretch", hide_index=True)

    if st.button("Save Polygon Results", key="save_polygon_results"):
